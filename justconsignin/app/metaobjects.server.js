...
async function ensureDefinition(admin, definition) {
  const existingData = await graphql(admin, GET_DEFINITION, { type: definition.type });
  const existing = existingData.metaobjectDefinitionByType;

  if (!existing) {
    const createData = await graphql(admin, CREATE_DEFINITION, { definition });
    assertNoUserErrors(createData.metaobjectDefinitionCreate, `Could not create ${definition.name}`);
    return { id: createData.metaobjectDefinitionCreate.metaobjectDefinition.id, type: definition.type, action: "created" };
  }

  const existingKeys = new Set(existing.fieldDefinitions.map((field) => field.key));
  const missingFields = definition.fieldDefinitions.filter((field) => !existingKeys.has(field.key));
...
export async function ensureMetaobjectsInstalled(admin, shop = "unknown shop") {
  try {
    ...
    const itemDefinition = {
      ...DEFINITIONS[1],
      fieldDefinitions: DEFINITIONS[1].fieldDefinitions.map((field) => ({
        ...field,
        validations: field.validations?.map((validation) => ({
          ...validation,
          value: validation.value === "__CONSIGNOR_DEFINITION_ID__"
            ? consignorResult.id
            : validation.value,
        })),
      })),
    };
    results.push(await ensureDefinition(admin, itemDefinition));
    console.log(`Metaobject setup completed for ${shop}`, results);
    return { ok: true, results, errors: [] };
  } catch (error) {
    console.error(`Metaobject setup failed for ${shop}:`, error);
    return { ok: false, results: [], errors: [{ message: error.message }] };
  }
}
